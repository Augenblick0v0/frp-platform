...
```bash
VERSION=0.1.0 ./client/packaging/windows/build-windows.sh
```

如果本机已安装 `makensis`，会额外输出：

```text
dist/windows/FrpTunnelClient-0.1.0-setup.exe
```

如果未安装 `makensis`，仍会输出：

```text
dist/windows/FrpTunnelClient-0.1.0-windows-amd64.zip
```
...
```powershell
.\client\packaging\windows\build-windows.ps1 -Version 0.1.0 -FrpcWindowsPath C:\path\to\frpc.exe
```

## Linux 客户端

```bash
VERSION=0.1.0 ./client/packaging/linux/build-linux.sh
```
...
```text
dist/linux/FrpTunnelClient-0.1.0-linux-amd64.tar.gz
```

安装：

```bash
tar -xzf FrpTunnelClient-0.1.0-linux-amd64.tar.gz
cd frp-client
sudo ./install.sh
