...
```bash
VERSION=0.1.4 ./client/packaging/windows/build-windows.sh
```
...
```powershell
.\client\packaging\windows\build-windows.ps1 -Version 0.1.4 -FrpcWindowsPath C:\path\to\frpc.exe
```

## Linux 客户端

```bash
VERSION=0.1.4 ./client/packaging/linux/build-linux.sh
```
...
```bash
tar -xzf FrpTunnelClient-0.1.4-linux-amd64.tar.gz
cd frp-client
sudo ./install.sh
