...
export default function App() {
  ...
  const [state, setState] = useState({ loading: true, loadError: '', me: null, topology: null, sub: null, traffic: null, tunnels: [], plans: [], nodes: [], clientConfig: null });
  ...
  async function load() {
    ...
    try {
      ...
      const [me, topology, sub, traffic, tunnels, plans, nodes, clientConfig] = await Promise.all([
        api.get('/api/auth/me'), optional(api.get('/api/user/topology'), null), optional(api.get('/api/user/subscription'), null), optional(api.get('/api/user/traffic'), null), optional(api.get('/api/tunnels'), []), optional(api.get('/api/user/plans'), []), optional(api.get('/api/user/nodes'), []), optional(api.get('/api/client/tunnels'), null)
      ]);
      setState({ loading: false, loadError: partial ? '部分数据加载失败，请刷新重试' : '', me, topology, sub: sub || topology?.subscription, traffic: traffic || topology?.traffic, tunnels, plans, nodes: nodes?.length ? nodes : (topology?.nodes || []), clientConfig });
  ...
  return <AppShell title={routes[current].title} subtitle={routes[current].subtitle} brand="FRP User" brandSub="用户控制台" menuItems={menuItems} selectedKey={current} onSelect={k => { location.hash = k; setRoute(k); }} onRefresh={load} userLabel={state.me?.email} onLogout={() => { api.setToken(''); location.hash = 'login'; }} storageKey="userSidebarCollapsedV4">
    {state.loadError && <Alert type="warning" showIcon message={state.loadError} style={{ marginBottom: 16 }} />}
  ...
  return <div className="create-layout">
    <Card bordered={false} className="page-card create-main-card" title="创建隧道" extra={<Tag color="blue">{protocol.toUpperCase()}</Tag>}>
      <Form layout="vertical" form={form} initialValues={{ local_host: '127.0.0.1', local_port: 80 }}>
        ...
        {step === 0 && <div className="form-section"><Typography.Title level={4}>基础信息</Typography.Title><Row gutter={16}><Col xs={24} md={12}><Form.Item name="name" label="隧道名称" rules={[{ required: true, message: '请输入隧道名称' }]}><Input placeholder={protocolMeta[protocol].placeholder} /></Form.Item></Col><Col xs={24} md={12}><Form.Item name="node_id" label="节点"><Select allowClear placeholder="自动选择或指定节点" options={(state.nodes || []).map(n => ({ value: n.id, label: `${n.name} / ${n.node_kind==='narwhal_nat'?'NAT':'FRPS'} / ${n.status || 'unknown'}` }))} /></Form.Item></Col></Row><Row gutter={16}><Col xs={24} md={12}><Form.Item name="local_host" label="本地地址" rules={[{ required: true, message: '请输入本地地址' }]}><Input placeholder="127.0.0.1" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name="local_port" label="本地端口" rules={[{ required: true, message: '请输入本地端口' }]}><InputNumber min={1} max={65535} style={{ width: '100%' }} /></Form.Item></Col></Row></div>}
...
function Nodes({ state }) { return <div className="card-grid">{state.nodes.map(n => <Card key={n.id} bordered={false} className="page-card" title={<Space>{n.name}<Tag color={n.node_kind==='narwhal_nat'?'purple':'blue'}>{n.node_kind==='narwhal_nat'?'NAT':'FRPS'}</Tag></Space>} extra={<StatusBadge status={n.status} />}><Descriptions column={1} size="small" items={[{ label: '公网入口', children: n.node_kind==='narwhal_nat' ? (n.nat_entry_host || n.server_addr || '-') : (n.frp_entry_domain || '-') }, { label: 'frps', children: `${n.server_addr || '-'}:${n.frp_server_port || '-'}` }, { label: 'TCP', children: `${n.tcp_port_start}-${n.tcp_port_end}` }, { label: 'UDP', children: `${n.udp_port_start}-${n.udp_port_end}` }, { label: '说明', children: n.node_kind==='narwhal_nat' ? '新增 TCP/UDP 隧道会自动绑定 NAT 端口转发' : '公网 FRPS 直连节点' }, { label: '最后在线', children: formatTime(n.last_seen_at) }]} /></Card>)}</div>; }
